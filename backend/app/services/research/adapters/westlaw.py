"""WestlawAdapter -- stub for Westlaw commercial research tool.
...
class WestlawAdapter(HTTPAdapter):
    """Westlaw research adapter stub.

    Raises NotConfiguredError on all operations until API credentials
    are configured. Ready for future implementation.

    Args:
        api_key: Westlaw API key (required for operation).
        base_url: Westlaw API base URL.
    ...
    async def discover(self, query: ResearchQuery) -> list[ResearchResult]:
        """Search Westlaw -- requires API credentials."""
        raise NotConfiguredError("Westlaw adapter requires API credentials. Configure via organization settings.")

    async def fetch_authority(self, citation: str) -> ResearchResult | None:
        """Fetch from Westlaw -- requires API credentials."""
        raise NotConfiguredError("Westlaw adapter requires API credentials.")

    async def verify_citation(self, citation: str) -> dict[str, Any]:
        """Verify via Westlaw -- requires API credentials."""
        raise NotConfiguredError("Westlaw adapter requires API credentials.")
