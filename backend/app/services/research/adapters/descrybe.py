"""DescrybeAdapter -- stub for Descrybe commercial research tool.
...
class DescrybeAdapter(HTTPAdapter):
    """Descrybe research adapter stub.

    Raises NotConfiguredError on all operations until API credentials
    are configured. Ready for future implementation.

    Args:
        api_key: Descrybe API key (required for operation).
        base_url: Descrybe API base URL.
    ...
    async def discover(self, query: ResearchQuery) -> list[ResearchResult]:
        """Search Descrybe -- requires API credentials."""
        raise NotConfiguredError("Descrybe adapter requires API credentials. Configure via organization settings.")

    async def fetch_authority(self, citation: str) -> ResearchResult | None:
        """Fetch from Descrybe -- requires API credentials."""
        raise NotConfiguredError("Descrybe adapter requires API credentials.")

    async def verify_citation(self, citation: str) -> dict[str, Any]:
        """Verify via Descrybe -- requires API credentials."""
        raise NotConfiguredError("Descrybe adapter requires API credentials.")
